    ...
    return (
        <div className='stat-tracker-mod-rows'>
        {
            rows.map(({ position, row, canDisplayProgression }) => {
                ...
                const rowClass = `stat-tracker-mod-row ${position <=3 && "top-position-3 pos-" + position} ${(expandedPlayer == account.slug) && "expanded"} ${disableUnclickableRows && ((account.link) ? 'highlight-on-hover' : 'disable-on-hover')}`;
